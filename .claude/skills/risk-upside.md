...
- `MarketData.get_short_interest(ticker)` — normalized short-interest snapshot (synchronous). Returns a `ShortInterest` dict with keys: `symbol`, `short_percent_of_float` (fraction 0.0–1.0, e.g. `0.21` = 21%), `days_to_cover`, `shares_short`, `as_of_date` (ISO date), `prior_shares_short`, `prior_short_percent_of_float` (may be `None` on the yfinance path), `prior_as_of_date`, and `source` (`"fmp"` or `"yfinance"`). Returns `{}` when no data is available from either provider. FMP-primary / yfinance-fallback is handled internally — the skill does not manage providers or fallbacks.
...
4. **Short-side dynamics** — pull short-interest data and apply the escalation gate below. This step ALWAYS runs; there is no skip condition.
   ...
   **Fetch:** call `MarketData.get_short_interest(ticker)` (synchronous). If it returns `{}`, record "Short-interest data unavailable" and proceed to step 5. Otherwise extract: `short_percent_of_float` (a fraction — multiply by 100 for display, e.g. `0.21` → "21%"), `days_to_cover`, `shares_short`, `as_of_date`, `prior_shares_short`, `prior_short_percent_of_float` (may be `None`), and `prior_as_of_date`.

   **Trend/delta:** if `prior_shares_short` is present (i.e., the result contains a `prior_as_of_date`), compute the period-over-period change in absolute short interest as `(shares_short − prior_shares_short) / prior_shares_short`. Note the period spanned by comparing `as_of_date` to `prior_as_of_date` (typically ~30 days, the FINRA settlement cadence — do not assume 90 days). Do NOT use `prior_short_percent_of_float` for the trend calculation, because it may be `None` on the yfinance path; base the trend condition entirely on `shares_short` vs. `prior_shares_short`. If no prior data point is present, skip the trend condition gracefully.
   ...
   **Escalation gate — default vs. escalated behavior:**
   ...
   - **ESCALATE to substantive bear-case narrative + bear-case PT input** when ANY of the following thresholds is met:
     - `short_percent_of_float` > ~0.10 (i.e., more than ~10% of float sold short), OR
     - `days_to_cover` > ~5, OR
     - `shares_short` has risen > ~50% relative to `prior_shares_short` over the reported prior-to-current period (only when a prior data point is available).
